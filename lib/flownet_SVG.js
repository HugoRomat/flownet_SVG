...
function graph(id, svg) {
  var newGraph = new flownet_svg_graph(id, svg);
  return newGraph;
}
...
flownet_svg_graph.prototype.links = function (data) {
  this.groupGraph.select('#links').append('g').attr('id', 'edges').selectAll('g').data(data).enter().append('g').attr('id', function (d, i) {
    if (d.id) return 'link_' + d.id;
    return 'link_' + i;
  }).each(function (d) {
    d3.select(this).append('path').attr('id', function (d, i) {
      if (d.id) return 'edge_' + d.id;
      return 'edge_' + i;
    }).attr('fill', 'none');
  });
  this.groupGraph.select('#links').append('g').attr('id', 'particules').selectAll('g').data(data).enter().append('g').attr('id', function (d, i) {
    if (d.id) return 'link_' + d.id;
    return 'link_' + i;
  }).each(function (d) {
    d3.select(this).append('g').attr('id', 'mainPath_' + d.id).append('path').attr('id', function (d, i) {
...
flownet_svg_graph.prototype.nodes = function (nodes) {
  this.groupGraph.select('#nodes').selectAll('circle').data(nodes).enter().append('circle').attr('id', function (d, i) {
...
flownet_svg_graph.prototype.nodes_properties = function (property, value) {
  ...
  var svgProp = convertNodePropertyToSVG(property);
...
function updateLinkPoints(link) {
  d3.select(link).selectAll('path').attr('d', function (d) {
...
flownet_svg_graph.prototype.links_properties = function (property, value) {
  ...
  if (property === 'interpolation') {
    this.groupGraph.select('#links').selectAll('g').filter(function (d) {
      ...
      var newinterpol = interpolateLink(typeof value === 'function' ? value(d) : value)(d.points);
...
function cutPathForGate(link, edge, particule, gate, offset) {
  ...
  for (var i = 0; i < 41; i++) {
    ...
    if (particule.getTotalLength() / 40 * i < switchDot) dotListBeforeCut.push({ x: dot.x, y: dot.y });else {
      if (!switchDuplicate) {
  ...
  newParticule = d3.select(link).insert('path', nextElelment ? '#' + nextElelment.getAttribute('id') : null).attr('id', function (d) {
    return 'particules_x';
  }).attr('gate', gate.type).attr('fill', 'none').attr('d', function (d) {
    return interp(dotListAfterCut);
  ...
  }).attr('stroke-dasharray', particule.getAttribute('stroke-dasharray')).attr('stroke-dashoffset', function (d) {
    previousDashOffset = Number(particule.getAttribute('stroke-dashoffset'));
    d.previousDashOffset = (particule.getTotalLength() + previousDashOffset) % particule.getAttribute('stroke-dasharray').split(',').reduce(function (a, b) {
      return Number(a) + Number(b);
    }, 0);
    return (particule.getTotalLength() + previousDashOffset) % particule.getAttribute('stroke-dasharray').split(',').reduce(function (a, b) {
      return Number(a) + Number(b);
    }, 0);
  });
  ...
  for (var i = 1; i < link.childNodes.length; i++) {
    if (link.childNodes[i].getAttribute('id') === particule.getAttribute('id')) indexOfParticule = i;
  }
...
function propagateGate(link, gate) {
  ...
  if (gate.type === 'color') link.setAttribute('stroke', gate.newValue);else if (gate.type === 'height') link.setAttribute('stroke-width', gate.newValue);
...
flownet_svg_graph.prototype.link_add_gate = function (type, newValue, position) {
  ...
  if (type === 'color') {
    this.groupGraph.select('#links').selectAll('g').filter(function (d, i) {
      ...
      dataBind = null;
      d3.select(this).select('path').each(function (d) {
        dataBind = d;
      });
...
function computeParticleRender(motionModel, freq, spacing, speed, pattern) {
  ...
  var newPattern = JSON.parse('[' + pattern + ']');
  var newSpeed = Number(speed);
  var newSpacing = Number(spacing);
  console.log(newPattern, newSpeed, newSpacing);
  var spaceArray = [];
  var indexSpaceArray = 0;
      ...
      if (d.frequency && d.speed) {
        ...
        if (d.pattern) {
          if (d.patternType === 'absolute') {
            ...
            total = d.pattern.reduce(function (a, b) {
              return a + b;
            }, 0);
            adjust = d.width / total;
            newPattern = d.pattern.map(function (x) {
              return x * adjust;
            });
            if (space - d.width > 0) return newPattern.concat(space - d.width);
  ...
  switch (property) {
    case 'computationalMethod':
      this.groupGraph.select('#links').selectAll('path').filter(function (d, i) {
        ...
        return computeParticleRender(this.getAttribute('computationalMethod'), this.getAttribute('frequency'), this.getAttribute('spacing'), this.getAttribute('speed'), this.getAttribute('pattern'));
      });
      break;
...
function startTransitionFPS(link, FPS) {
  ...
  offset = patternArray.map(function (x) {
    return Number(x);
  }).reduce(function (a, b) {
    return a + b;
  }, 0);
  ...
  function fpsTransition(link, delay, step, limit, start, offset, adjust) {
    if (step === limit) {
      link.transition().delay(delay).duration(0).ease(d3.easeLinear).attr('stroke-dashoffset', offset + adjust).on('start', function () {
        fpsTransition(link, delay, 0, limit, start - offset, offset - offset, adjust);
      });
    } else {
      link.transition().delay(delay).duration(0).ease(d3.easeLinear).attr('stroke-dashoffset', start + adjust + (offset + adjust - (start + adjust)) / limit * step).on('start', function () {
...
function startTransitionSVG(link) {
  ...
  var dashOffset = Number(link.attr('stroke-dashoffset'));
  var patternArray = link.attr('stroke-dasharray').split(',');
  var offset = patternArray.map(function (x) {
    return Number(x);
  }).reduce(function (a, b) {
    return a + b;
  }, 0);
  var offsetMinusFrequency = offset; // - Number(patternArray[patternArray.length-1])
  function motionDash(start, offset, adjust) {
    ...
    }).ease(d3.easeLinear).attr('stroke-dashoffset', offset + adjust).on('end', function () {
...
flownet_svg_graph.prototype.particule_launcher = function (idLink) {
  function startTransitionSVG_fire(link, full, delay, step) {
    var patternArray = link.attr('stroke-dasharray').split(',').map(function (x) {
      return Number(x);
    });
    var lengthOfLastPart = patternArray[patternArray.length - 1];
    ...
    link.transition().delay(delay).duration(function (d) {
    ...
    }).ease(d3.easeLinear).attrTween('stroke-dasharray', function (d) {
      var last = d3.interpolate(patternArray[patternArray.length - 1], 0);
      var first = d3.interpolate(0, patternArray[patternArray.length - 1]);
      ...
      if (full && full === true) {
        var newArray = link.attr('stroke-dasharray').split(',').map(function (x) {
          return Number(x);
        });
        newArray.splice(newArray.length - 1, 1);
        link.attr('stroke-dasharray', newArray);
        var nbToDestroy = Number(link.attr('numberToDestroy')) - 1;
        link.attr('numberToDestroy', nbToDestroy);
        if (d.onendfun) d.onendfun(d.data, 0);
        if (nbToDestroy <= 0) {
          d3.select(link._groups[0][0].parentNode).remove();
        } else startTransitionSVG_fire(link, false, 0, step += 1);
      } else {
        var _newArray = link.attr('stroke-dasharray').split(',').map(function (x) {
          return Number(x);
        });
        _newArray.splice(_newArray.length - 1, 1);
        link.attr('stroke-dasharray', _newArray);
  ...
  function computeInteractiveParticleRender(speed, actualPattern, pattern, total_length, actualDashOffset) {
    ...
    var actualDashOffsetCopy = actualDashOffset;
    var toRemove = 0;
    var newPattern = JSON.parse('[' + pattern + ']');
    var newSpeed = Number(speed);
    if (actualPattern === null) {
      var _offset = newPattern.map(function (x) {
        return Number(x);
      }).reduce(function (a, b) {
        return a + b;
      }, 0);
      return newPattern.concat(total_length - _offset);
    }
    actualPattern = actualPattern.split(',').map(function (x) {
      return Number(x);
    });
  ...
  particule_launcher.prototype.fire_particule = function () {
    for (var i = 0; i < this.waiting.length; i++) {
      this.fired.push(d3.select(this.entryPoint).append('g').selectAll('path').data([this.waiting[i]]).enter().append('path').attr('id', this.waiting[i].id).attr('fill', 'none').attr('d', d3.select('#' + this.d).attr('d')).attr('stroke', this.waiting[i].color[0]).attr('stroke-width', this.waiting[i].height[0]).attr('pattern', this.waiting[i].pattern).attr('speed', this.waiting[i].speed[0]).attr('delay', this.waiting[i].delay).attr('stroke-dasharray', function (d) {
        var res = computeInteractiveParticleRender(this.getAttribute('speed'), this.getAttribute('stroke-dasharray'), this.getAttribute('pattern'), this.getTotalLength(), this.getAttribute('stroke-dashoffset'));
        this.setAttribute('numberToDestroy', res.length / 2);
        var offset = res.reduce(function (a, b) {
          return a + b;
        }, 0) - res[res.length - 1];

...
flownet_svg_graph.prototype.selectNode = function (id) {
  let selected = this.groupGraph.select("#nodes").selectAll("circle").filter(function () {
    return this.getAttribute("id").split('_')[1].includes(id);
  });
  return selected;
};
...
function updateLinkPoints(link) {
  __WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](link).selectAll('path').attr('d', function (d) {
    return interpolateLink(__WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](link).attr('interpolation'))(JSON.parse(__WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](link).attr('points')));
  });
}
...
function cutPathForGate(link, edge, particule, gate, offset) {
  ...
  curve = __WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](link).attr('interpolation');
  interp = interpolateLink(curve);
  /* d3.select(edge).attr('d', function(){
    return interp(dotListBeforeCut)
  })*/
  __WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](particule).attr('d', function () {
    return interp(dotListBeforeCut);
  });
  /*
  ...
  newParticule = __WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](link).insert('path', nextElelment ? '#' + nextElelment.getAttribute('id') : null).attr('id', function (d) {
    return 'particules_x';
  ...
  __WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](link).selectAll('path').attr('id', function (d, i) {
    if (i === 0) {
...
flownet_svg_graph.prototype.link_add_gate = function (type, newValue, position) {
  ...
  if (type === 'color') {
    this.groupGraph.select('#links').selectAll('g').filter(function (d, i) {
      ...
      dataBind = null;
      __WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](this).select('path').each(function (d) {
        dataBind = d;
      });
...
flownet_svg_graph.prototype.start = function () {
  ...
  if (FPS === 'auto') {
    this.groupGraph.select('#links').select('#particules').selectAll('g').selectAll('path').filter(function (d) {
      ...
      startTransitionSVG(__WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](this));
    });
  } else {
    this.groupGraph.select('#links').selectAll('g').select('g').selectAll('path').filter(function (d) {
      return this.getAttribute('id').includes('particules');
    }).each(function (d) {
      startTransitionFPS(__WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](this), FPS);
    });
  }
  return this;
}; // OK
  ...
  particule_launcher.prototype.fire_particule = function () {
    for (var i = 0; i < this.waiting.length; i++) {
      this.fired.push(__WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](this.entryPoint).append('g').selectAll('path').data([this.waiting[i]]).enter().append('path').attr('id', this.waiting[i].id).attr('fill', 'none').attr('d', __WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */]('#' + this.d).attr('d')).attr('stroke', this.waiting[i].color[0]).attr('stroke-width', this.waiting[i].height[0]).attr('pattern', this.waiting[i].pattern).attr('speed', this.waiting[i].speed[0]).attr('delay', this.waiting[i].delay).attr('stroke-dasharray', function (d) {
        let res = computeInteractiveParticleRender(this.getAttribute('speed'), this.getAttribute('stroke-dasharray'), this.getAttribute('pattern'), this.getTotalLength(), this.getAttribute('stroke-dashoffset'));
        ...
        startTransitionSVG_fire(__WEBPACK_IMPORTED_MODULE_0_d3__["m" /* select */](this), false, Number(this.getAttribute('delay')), 0);
      }));
